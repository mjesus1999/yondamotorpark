...
BEGIN
    SELECT
        ...
        -- Ubicación completa (provincia/distrito/dirección SOLO SI EXISTE)
        CASE
            WHEN cli.tipocliente = 'P' THEN 
                CONCAT(
                    ...
                    CASE 
                        WHEN p.direccion IS NOT NULL AND TRIM(p.direccion) != '' 
                        THEN CONCAT(' / ', p.direccion)
                        ELSE ''
                    END
                )
            ELSE 
                CONCAT(
                    ...
                    CASE 
                        WHEN e.direccion IS NOT NULL AND TRIM(e.direccion) != '' 
                        THEN CONCAT(' / ', e.direccion)
                        ELSE ''
                    END
                )
        ...
        CASE 
            WHEN cli.tipocliente = 'P' THEN p.latitud
            ELSE e.latitud
        END AS latitud,
        
        CASE 
            WHEN cli.tipocliente = 'P' THEN p.longitud
            ELSE e.longitud
        END AS longitud,
    ...
    GROUP BY 
        ...
        p.latitud, p.longitud, e.latitud, e.longitud
