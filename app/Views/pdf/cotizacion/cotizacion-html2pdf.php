...
<head>
  ...
  <style>
    ...
    function showDownloadModal(pdfDocGenerator, filename) {
      ...
      modalOverlay.innerHTML=` <div class="modal"><p style="margin: 0 0 20px 0; font-size: 14px;">PDF generado correctamente. ¿Desea descargarlo? </p><div><button id="btn-download-pdf">Descargar</button><button id="btn-cancel-download">Cancelar</button></div></div>`;
...
<body>
  ...
  <script>
    ...
    const cotId = pathSegments[pathSegments.length - 1]; // Último segmento de la URL
    ...
    function showPDFPreview(pdfDocGenerator, filename) {
      ...
      // Generar el PDF como blob y crear URL para el iframe
    ...
    async function generatePDFCotizacion() {
      ...
      try {
        ...
        // Mostrar vista previa o modal según el modo
        if (isPreview) {
