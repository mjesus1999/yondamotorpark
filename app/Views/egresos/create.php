                ...
                <div class="card-body p-4">
                    <form id="egresoForm">
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                <label for="colaborador" class="form-label fw-semibold">Colaborador que Solicita <span class="text-danger">*</span></label>
                                <input list="colaboradores-list" id="colaborador" name="idsolicitante" class="form-control" placeholder="Escriba o seleccione un colaborador" required>
                        ...
                        <div id="comprobanteFormContainer" style="display: none;" class="card border-primary p-4 mt-4">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6 mb-3">
                                    <label for="proovedor" class="form-label fw-semibold">Proveedor <span class="text-danger">*</span></label>
                                    <input list="proovedores-list" id="proovedor" name="idproovedor" class="form-control" placeholder="Escriba o seleccione un proveedor">
                                    <datalist id="proovedores-list"></datalist>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-md-6 mb-3">
...
<script>
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        // Manejar la visibilidad del formulario de comprobante
        requiereComprobanteCheckbox.addEventListener('change', () => {
            comprobanteFormContainer.style.display = requiereComprobanteCheckbox.checked ? 'block' : 'none';
        });
        ...
        egresoForm.addEventListener('submit', async (e) => {
            ...
            // Recopilar datos y realizar validaciones en el frontend
            ...
            if (requiereComprobante) {
                const tipoDoc = document.getElementById('tipodoc').value;
                const proovedorInput = document.getElementById('proovedor').value.trim();
                const proovedorOption = Array.from(document.getElementById('proovedores-list').options)
                    .find(option => option.value === proovedorInput);
                const proovedorId = proovedorOption ? proovedorOption.dataset.id : null;
                const serie = document.getElementById('serie').value.trim();
                const numDocumento = document.getElementById('numdocumento').value.trim();
                const montoComprobante = document.getElementById('monto_comprobante').value.trim();
                const archivoComprobante = document.getElementById('rutacomprobante').files[0];
                ...
                // Anexar los datos del comprobante al mismo FormData
                formData.append('idproovedor', proovedorId);
                formData.append('tipodoc', tipoDoc);
                formData.append('serie', serie);
                formData.append('numdocumento', numDocumento);
        ...
        async function loadData(url, datalistId) {
            try {
                const response = await fetch(url);
                const result = await response.json();
                if (result.status) {
                    const element = document.getElementById(datalistId);
                    element.innerHTML = '';
                    result.data.forEach(item => {
                        const option = document.createElement('option');
                        if (datalistId === 'colaboradores-list') {
                            option.value = item.colaborador;
                            option.dataset.id = item.idcolaborador;
                        } else if (datalistId === 'proovedores-list') {
                            option.value = `${item.nombrecomercial} - (${item.ruc})`;
                            option.dataset.id = item.idproovedor;
                        } else if (datalistId === 'concepto') {
                            option.value = item.idconceptoegreso;
                            option.textContent = item.descripcion;
                        }
        ...
        loadData('/api/egreso/conceptos', 'concepto');
        loadData('/api/egreso/colaboradores', 'colaboradores-list');
        loadData('/api/egreso/proovedores', 'proovedores-list');
