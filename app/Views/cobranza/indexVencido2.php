    ...
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        try {
                ...
                columns: [
                    ...
                    { 
                        ...
                        formatter: function(cell) {
                            ...
                            const direccion = cell.getValue() || 'Sin dirección';
                            
                            const tieneUbicacion = (data.latitud && data.longitud) || 
                                                  (direccion && direccion !== 'Sin dirección' && !direccion.includes('Sin provincia'));
                            
                            if (tieneUbicacion) {
                                return `
                                    <div class="d-flex align-items-center gap-2">
                                        <span class="flex-grow-1">${direccion}</span>
                                        <button class="btn btn-sm btn-outline-primary btn-ver-mapa" 
                                                data-contrato="${data.idcontrato}"
                                                data-cliente="${escapeHtml(data.cliente)}"
            ...
            contTabla.addEventListener('click', async function (e) {
                // 1. Verificar si es clic en botón de mapa
                const btnMapa = e.target.closest('.btn-ver-mapa');
                if (btnMapa) {
                    const datos = {
                        nombre: btnMapa.dataset.cliente,
                        direccion: btnMapa.dataset.direccion,
                        lat: btnMapa.dataset.lat,
                        lng: btnMapa.dataset.lng
                    };
                    
                    if (typeof window.abrirMapaViewer === 'function') {
                        window.abrirMapaViewer(datos);
                    } else {
                        mostrarNotificacion('El visor de mapas no está disponible', 'warning');
                    }
                    return;
                }

                // 2. Verificar si es clic en "Estado de pagos"
                ...
                // 3. Verificar si es clic en botones PDF
            ...
            datos.forEach(row => {
                ...
                const tieneUbicacion = (row.latitud && row.longitud) || 
                                      (row.ubicacion_cliente && row.ubicacion_cliente !== 'Sin dirección' && !row.ubicacion_cliente.includes('Sin provincia'));
                            ...
                            <div class="accordion-body">
                                <ul class="list-group list-group-flush">
                                    ...
                                    <li class="list-group-item">
                                        ...
                                        ${tieneUbicacion ? `
                                            <button class="btn btn-sm btn-outline-primary btn-ver-mapa float-end" 
                                                    data-contrato="${row.idcontrato}"
                                                    data-cliente="${escapeHtml(row.cliente)}"
                                                    data-direccion="${escapeHtml(row.ubicacion_cliente)}"
                                                    data-lat="${row.latitud || ''}"
                                                    data-lng="${row.longitud || ''}"
                                                    title="Ver en mapa">
                                                <i class="bi bi-geo-alt-fill"></i>
                                            </button>
                                        ` : ''}
...
<script>
    // Inicializar el visor de mapas cuando se cargue Google Maps
    if (typeof google !== 'undefined' && google.maps && google.maps.loadViewer) {
        google.maps.loadViewer(['maps', 'geocoding', 'marker'], initMapaViewer);
    } else {
        // Fallback: esperar a que se cargue
