...
<style>
    ...
    /* Asegura que íconos y textos dentro de tarjetas con gradiente sean legibles */
    /* .card.bg-gradient .card-body,
    .card.bg-gradient .card-body i,
    .card.bg-gradient .card-title,
    .card.bg-gradient .card-text {
        color: #fff !important;
    } */
...
<?php
// Seguridad: evitar undefined vars
...
// Normalizar: soportar dos estructuras (clasificado o plano)
$morososClasificados = [
    '5-dias' => [],
    '2-semanas' => [],
    '1-mes' => []
...
<div class="container-fluid p-4">
    ...
    <!-- Lista de morosos (render dinámico) -->
    <div class="row" id="listaMoresos">
        ...
        foreach (['5-dias', '2-semanas', '1-mes'] as $categoria):
            ...
            // alerta por categoría (se deja en el DOM siempre, pero controlamos visibilidad por PHP y JS)
        ...
        <!-- Si no hay morosos en ninguna categoría (todos vacíos) mostramos mensaje general -->
...
<!-- Modal para seguimiento (mantener igual que ya tienes) -->
...
<script>
    ...
    function filtrarPor(categoria) {
        ...
        if (categoria === 'todos') {
            ...
            // Ocultar alertas por categoría (cuando vemos "Todos" no queremos esas alertas)
        ...
        // Mostrar solo las tarjetas de la categoría solicitada (ocultar las demás)
        items.forEach(item => {
            item.style.display = (item.dataset.categoria === categoria) ? 'block' : 'none';
        });

        // Para las alertas por categoría: mostrar solo la alerta de la categoría solicitada (si corresponde)
    ...
    // Ver historial abre la vista del historial (puede ser una URL o modal)
    function verHistorial(id) {
        window.location.href = '/creditos/historial/' + id;
    }

    // Contactar cliente (tel:)
    function contactarCliente(telefono) {
        if (confirm('¿Deseas llamar al número ' + telefono + '?')) {
