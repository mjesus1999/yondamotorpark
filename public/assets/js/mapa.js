...
class MapaSelector {
  ...
  init() {
    ...
    if (typeof L === "undefined") {
      console.error(
        "Leaflet no está cargado. Asegúrate de incluir la librería."
      );
      return;
    }
  ...
  inicializarMapa() {
    ...
    const latInicial = inputLatitud
      ? parseFloat(inputLatitud.value) || this.options.latInicial
      : this.options.latInicial;
    const lngInicial = inputLongitud
      ? parseFloat(inputLongitud.value) || this.options.lngInicial
      : this.options.lngInicial;

    // Crear mapa
    this.mapa = L.map(this.options.mapaId).setView(
      [latInicial, lngInicial],
      this.options.zoomInicial
    );
    ...
    if (
      latInicial !== this.options.latInicial ||
      lngInicial !== this.options.lngInicial
    ) {
  ...
  actualizarMarcador(lat, lng) {
    ...
    // Capturar dirección textual
    this.obtenerDireccionPorCoordenadas(lat, lng);
  }

  actualizarCoordenadasSeleccionadas(lat, lng) {
    const inputLatSeleccionada = document.getElementById(
      this.options.inputLatitudSeleccionadaId
    );
    const inputLngSeleccionada = document.getElementById(
      this.options.inputLongitudSeleccionadaId
    );
  ...
  async obtenerDireccionPorCoordenadas(lat, lng) {
    try {
      ...
      if (data && data.address) {
        const { name, amenity, road, house_number, neighbourhood } =
          data.address;

        // Construir dirección más específica 
        const partes = [];
        if (name) partes.push(name);
        else if (amenity) partes.push(amenity);

        if (road) partes.push(road);
        if (house_number) partes.push(`#${house_number}`);
        if (neighbourhood) partes.push(neighbourhood);

        const direccionEspecifica = partes.join(", "); // Convertir los elmentos del array en string separados por coma

        const inputDireccion = document.getElementById(
          this.options.inputDireccion
        );
        if (inputDireccion)
          inputDireccion.value = direccionEspecifica || data.display_name;
      } else {
  ...
  confirmarUbicacion() {
    if (this.coordenadasSeleccionadas) {
      ...
      const inputLongitud = document.getElementById(
        this.options.inputLongitudId
      );

      if (inputLatitud)
        inputLatitud.value = this.coordenadasSeleccionadas.lat.toFixed(6);
      if (inputLongitud)
        inputLongitud.value = this.coordenadasSeleccionadas.lng.toFixed(6);
